...
demo.state6.prototype = {
    ...
    create: function () {
        ...
        balloons.forEach((b, i) => {
            ...
            balloons[i].events.onKilled.add(() => {
                this.score++;
            });
        ...
        this.cannonConfigurations = gameData.cannonConfigurations;    
        this.createWeapons();    
        levelData.cannonLocations.forEach(function (l) {
            var c = game.add.sprite(l.position.x, l.position.y, 'tw-assets');
            cannonConfig = this.cannonConfigurations.filter(function (c) {
                return c.id == l.id
            })[0];
            c.weaponId = l.id;
            c.nextFire = 0;
    ...
    createWeapons: function () {
        this.weapons = {};
        //get the different kinds of bullets in the game
        var bulletTypes = this.cannonConfigurations.map((c)=>{return c.weaponSpeed;});
        bulletTypes.forEach((b)=>{
            if (!this.weapons.hasOwnProperty(b)){
                var weapon = game.add.weapon(20, 'tw-assets', 4);
                ...
                this.weapons[c.weaponSpeed]=weapon; 
            }
        });
        console.log(weapons);
    },
    showExplosionAtSprite: function (sprite) {        
        fx.play('plop', 1.0);
    ...
    fire: function (cannon, target) {
        if (game.time.now > cannon.nextFire) {
            ...
            this.weapons[cannon.weaponSpeed].trackSprite(cannon, 0, 0, true); //basically from where to shoot with an offset for the turret            
            this.weapons[cannon.weaponSpeed].fireAtSprite(target);
        }
    },
    balloonHit: function (bullet, balloon) {
        this.showExplosionAtSprite(balloon);
        bullet.kill();
        balloon.kill();
    },
    update: function () {
